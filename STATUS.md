...
> ## ▶ 現在地（2026-09-23 23:44）: Y9 をカートに投入済み（JLC の品目番号は Y13）。決済なし。Y8 の 2 品目は施主の了承（「1 OK」）を受けてカートから削除した
>
> | 品目 | 番号 | 金額 | Y8 との差 |
> |---|---|---|---|
...
> - 記録: `gates/S9.json`（Y8 の記録は `y8_record` と history に残した）、スクショ 12 枚は `fab/jlc_cart_2026-09-23_y9/`。
> - Y8 の 2 品目（`Y8-10641515A` / `SMT026082960311`）はカートから削除済み（スクショ 12）。M5 と Ganglion の品目には触っていない。
> - **次（施主）**: Y13 の 2 行だけを決済する。
>   決済後は、JLC の Confirm Production file に 48 h 以内、Confirm Parts Placement に 72 h 以内に応答する。

> ## ✅ 2026-09-24: 新しい afe-adc J1 でも緑（lead 裁定 (a)）。DVDD 48/50 の配線の長さは Rev.B の候補（(b)、J1 の外）
>
> - ハーネス cyton-3a（`ca429f1`、Q508 でベタ免除を「経路がベタを通るとき」に限った版）の J1 を HEAD の盤面に当てた。**pass 0**。判定したのは 100 行で、宣言済みの例外 1 件（C_VCAP1 → pin 28）が効いている。stale 0。
> - ピンごとの実測（4 層 JLC04161H-7628）は次のとおり。AVDD 21/22 は C_AVDD_P31 まで F.Cu で 2.9〜3.9 mm、via 0。AVDD 19/56/59 は、ピンから 1.5〜2 mm の F 配線で via に入り、In2 の AVDD ベタ（281 mm²）に落ちる。TI SBAS499C Fig.80（p.73）と同じ「電源はベタで配る」作りなので、免除は正当。
> - **別件（(b)、Rev.B の候補）**: DVDD 48/50 は via 0 だが、最寄りの C_DVDD_P40 まで F.Cu で **10.6〜13.7 mm**（直線距離では 6.2〜8.3 mm）ある。経路の L の見積もりは 3〜14 nH で、m5stack パターン 2 の DVDD（J1 で赤）と同程度。J1 は via の本数しか数えないので緑になる。Rev.B では、**全電源ピンについて via 0・2 mm 以内に 100 nF を 1 個**置く（m5stack と同じ必須項目）。
> - 物理的な判断の要旨: fMOD = 1.024 MHz で、100 nF のリアクタンス 1.55 Ω に対し経路は 0.02〜0.09 Ω。ADC 自身の電流は fCLK 同期なので DC に畳まれる。入力換算ノイズへの影響は無いと判断した（MHz 帯の PSRR は DS に数値が無いので、推測を含む）。**火入れで確かめる**: `reports/bringup_checklist.md` の 8-5。
> - 盤面・fab・BOM/CPL・`gates/S9.json` は不変。Y13 の決済は止めない。
...
> | 2 | S7m（`scripts/71_mask_webs.py`）: 異ネットのパッド 22 組について、関係する 35 パッドのマスク拡張だけを下げた。値は (銅の間隔 − 0.10) / 2 で、全体の 0.0508 はそのまま。**基準 L**（`scripts/72_mask_web_gate.py`、Gerber、陰性対照つき） | 異ネットで堤が 0.10 未満の組は **0**。物理的に無理な組も 0。同ネットの 17 組は触っていない（`logs/mask_webs.json`） |
...
> ### 2026-09-23: afe-adc の J1（バイパスコンデンサと ADS のピンの間に via）と S3_sim の不安定を直した
>
> **J1**（S7j、`scripts/73_bypass_caps.py`）:
>
> | 部品（ピン） | 対処 | ピン→コンデンサ（前 → 後） | via | AVSS に戻るループ面積 | インダクタンスの目安 |
> |---|---|---|---|---|---|
> | C_VCAP4 1 µF（26） | ピン 26 の真下へ移動（C_VCAP1_H と同じ向き）。パッド 1 はピンの既存の F.Cu に乗り、パッド 2 は F.Cu の AVSS 配線（y 109.769）に乗る | 9.213 → **1.358 mm** | 2 → **0** | 18.60 → **6.40 mm²** | 約 7 → 約 0.7 nH |
> | C_VCAP3 1 µF（55） | C_VCAP3_H（100 nF）と位置を交換。同じ 0402 でパッドのネットも同じなので、銅は動かしていない | 5.190 → **2.100 mm** | 2 → **0** | 4.02 → **2.14 mm²** | 約 5 → 約 1 nH |
> | C_VCAP1 100 µF（28） | **動かしていない**（F.Cu だけで結ぶ経路が物理的にない。下記） | 12.665 → 16.548 mm | 4（うち C_VCAP1_H 側の 2 本は変わらない） | 14.72 → 14.73 mm² | — |
>
> インダクタンスは、0.2〜0.25 mm の配線を内層の面から約 0.2 mm 上に通したときの目安 0.5 nH/mm と、1.6 mm 厚を貫く via 1 本あたり約 1.3 nH で見積もった粗い値。
> `product.yaml` の `packs.afe-adc.ds_required_cap` に、DS が求める容量のコンデンサ（VCAP1: C_VCAP1、VCAP3: C_VCAP3、VCAP4: C_VCAP4、VREFP: C_VREFP_10u）を宣言した。
> ハーネスの afe-adc S5_layout の J1 は **3 件 → 1 件（C_VCAP1）**。
>
> **C_VCAP1 を F.Cu だけで結べない理由（数値）**:
> - ピン 28 の周り: 隣のピン 27 / 29 まで 0.22 mm。真下の C_VCAP1_H まで 0.208 mm、C_VCAP1_H と C_VCAP2 の間は 0.314 mm。
>   異ネットの間に 0.2 mm の配線を 1 本通すには 0.378 mm 要るので、どれも通れない。
> - その南側: 同じ F.Cu を AVSS の配線（y 109.769、x 146.92〜151.29）と VREFP の配線（y 111.496、x 145.92〜151.75）が、クラスタの幅いっぱいに横切っている。
> - 1206（本体 3.2 × 1.6 mm）をピン 28 の近くに置く場所がない。置くには C_VCAP1_H・C_VCAP2・C_VREFP_100n・C_VREFP_10u を並べ替える必要がある。
> - C_VCAP1_H（100 nF）はピン 28 から F.Cu で 1.358 mm、via なしのまま。
> - **lead 裁定（2026-09-23）: J1 の宣言された例外として受け入れた**（`packs.afe-adc.j1_accepted_exceptions`。理由は ACCEPTANCE の追記 6）。
>   ハーネス 1e232da（Q492）で J1 がこの宣言を読むようになり、**afe-adc S5_layout は 4/4**（C_VCAP1 は「宣言された例外」として通る）。
>
> **S3_sim**:
> - 原因: TI のモデルを読むために PSpice 互換モード（`ngbehavior=psa`）を使っていた。このモードでは ngspice 45.2 がデッキの先頭に自前の補助関数（`.func pwr` など）を差し込み、その解釈にときどき失敗していた。
> - 対処: TI のモデルを ngspice ネイティブの書き方に機械変換した写しを使い、互換モードは使わない（`sim/lib/pspice_native.py`。TI の原本はそのまま）。
>   VSWITCH は ngspice 自身の互換モードと同じ `pswitch` に写した（libngspice の変換テンプレートから読んだもの）。
>   元の互換モードでの実行と比べて、LM2664 の出力電圧・リップル、TPS72325 の出力電圧（EN あり / EN = 0 V）は **5 桁まで一致**。
> - 再試行は既定で 1 回（再試行しない）にした。10 回連続で **10/10 回とも 30/30**。
> - あわせて、実行ごとの一時ディレクトリが消されずにディスクを埋めていた不具合を直した。
>
> **ゲート**: 製品側は全て pass。ハーネス（0c78e9b）は S0_manifest 50/50・S6 16/16・S7 58/58・S8 52/52・S7_body 4/4・S7_viapad 16/16・S7_clearance 8/8。
> - DRC: error 0 / 未接続 0 / dangling 0。courtyard と silk の warning は部品移動の分だけ増えた。
> - via-in-pad の最小距離 0.1007 mm、異ネットのマスク堤 0.10 mm 未満は 0、ペーストのはみ出し 0。
> - IN1P〜IN8N の経路長の変化は最大 0.0013 mm。via は 230 本。
> - 新しい fab: zip `14304d82…`、`board.d356` `2fa13efe…`。**CPL は 3 行変わった**（C_VCAP3 / C_VCAP3_H / C_VCAP4、`2c68a8ef…`）。BOM は変わっていない。

> ### 2026-09-23: 再 QA（SHIP）の MINOR の最後の仕上げ
>
> | MINOR | 何をしたか | 結果 |
> |---|---|---|
> | 1（マスク堤が 0.09998 mm、13 組） | S7m の設計目標を **0.105 mm** に上げた。基準 L は 0.10 mm のまま判定し、別の行で 0.105 の設計目標も判定する | 異ネットで 0.105 未満は **0 組** |
> | 3（新しく足した銅が他ネットまで 0.092 mm） | S7v の配置で他ネットまで 0.127 mm を守らない段（0.10 / 0.0889 mm）を廃止した。固定の編集（`ROOM_EDITS`）も 0.127 mm で検証する。**D_ESD.3**: UART の 2 本をパッド列の間だけ 0.1016 mm に細くして北へ寄せた。**J1.4（USB_CC1）**: 0.46/0.30 mm の via に替えた。新しい検査 **S7n**（`scripts/74_new_copper_clearance.py`）で、今回足した配線・via・パッドを全部測る | 0.127 mm 未満は 2 件だけで、どちらも理由を記録した例外: **R_CC2.2 の via 0.120 mm**（パッド間 0.70 mm に入る最大。0.127 mm にするには via 径 0.446 mm が要るが、ドリル 0.30 mm で環幅 0.076 mm を守る最小径は 0.452 mm）、**ADS_RESET_N の幹線 0.1018 mm**（Y8 の銅を短くしただけで、この距離は取り込み時からのもの） |
> | 2（C_VCAP4 が詰まっている） | C_VCAP4 はピン 26 の下のまま（F.Cu 1.358 mm、via なし）。C_VREFP_10n を 0.10 mm 南へずらした。C_VCAP4 を上へ動かすとピン 26 まで 0.208 → 0.118 mm になり、回すと C_VCAP1_H まで 0.06 mm になるため | C_VCAP4.2 と C_VREFP_10n.2 のパッド間は 0.114 → **0.214 mm**、マスク開口は分かれた（堤 0.112 mm）。courtyard の干渉 4 件は残った: EasyEDA の 0402 の courtyard（1.93 × 1.17 mm）はピッチ 1.0 mm の並びでは必ず重なる。部品本体どうしの隙間は 0.35〜1.75 mm で、基準 H も pass |
> | 4 | STATUS の数字を 17 組に直した（上の表） | — |
>
> - **ゲート**: 製品側は全て pass（S7n を含む）。ハーネス 1e232da は S0 50/50・S6 16/16・S7 58/58・S8 52/52・S7_* すべて緑・**afe-adc S5_layout 4/4**。
>   DRC は error 0 / 未接続 0 / dangling 0。IN1P〜IN8N の経路長の変化は最大 0.0013 mm、via は 230 本、via-in-pad の最小距離 0.1007 mm。
> - **新しい fab**: zip `00dd48f4…`、`board.d356` `dcc35c27…`、CPL `f46e2d6e…`（前回から C_VREFP_10n の 1 行だけ変わった）。BOM は変わっていない。
