...
最終更新: 2026-08-29 12:10 / 担当: S0–S2 → S3・S4 → S5・S6 → S7・S8 → S9 → L7〜L9・H・S8 再出力・S9 再投入 → **独立機能レビュー反映（BOM のみ 4 件・銅箔不変）**
...
> ### ⚠ 2026-08-29: BOM が 4 件変わった。**カートの部品表を入れ直してから決済すること**
>
> 独立機能レビュー（`reports/functional_review_2026-08-29.md`）の指摘を反映して
> **BOM だけ** 4 件変更した。**Gerber・CPL・IPC-D-356 は生成日時の行を除いて前回と完全一致**
> なので **PCB（Y6）は入れ直さなくてよい**が、**PCBA 側の BOM は現在カートにある版が古い**。
>
> | designator | 旧 | 新 |
> |---|---|---|
...
>
> **総行数 30 行・総点数 133 点は不変。Extended は 13 行 → 14 行**（フィーダー費 +$3.07）。
> `C12530` は BOM から消える。理由・出典は `data/bom_fixes_2026-08-28.json` の `R29-1`〜`R29-5`。
...
| 製造データ | Gerber 14 ファイル / BOM 30 行 133 点 / CPL 133 行 / IPC-D-356 678 features |
