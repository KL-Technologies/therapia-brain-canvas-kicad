...
| 0-3 | **R_LED = 330 Ω のまま**（差し替えない）／LED は `C84268`（NationStar NCD0603Y1、**黄** Vf≈2.0 V） | (3.3−2.0)/330 ≈ **3.9 mA**。100 Ω 案は「緑 LED (Vf 3.3 V)」を前提とした旧レビューの記述で、**黄 LED では過電流になるので採らない**（`data/bom_fixes_2026-08-28.json` B3 / D_LED） |
...
| 0-6 | 2026-08-29 の BOM 改訂 4 件が発注 BOM に入っているか | `C_CM1P〜C_CM8N` = **C163508**（1 nF 50 V **C0G** 0603）/ `C_NLDO_OUT`・`C_LM_IN` = **C23733**（4.7 µF 10 V X5R 0402）/ `C_AVDD_P59` = **C52923**（1 µF 0402）。銅箔・CPL は不変（`reports/functional_review_2026-08-29.md`） |
...
| 5-5 | **ESP32 の U0RXD（IO3）のアイドル電圧をテスタで測る**（CH340C の TXD が繋がる側。UART が無通信のときの直流電圧） | **3.6〜3.9 V 程度に収まっていれば想定内**（下記） | **5.0 V に張り付く／4 V を大きく超える → その基板は長時間運用に回さない。** Rev.B まで評価用に留める |

> **なぜ 5-5 を測るのか（Rev.A の受容済みリスク）**: 本基板の CH340C は VCC = **5 V** で動いており、
> その TXD / DTR#・RTS# は **5 V ロジック**で ESP32 の U0RXD / EN / IO0 を叩く。
> ESP32 の絶対最大定格は 3.6 V なので、原理的には超過している。実際にはピンの ESD クランプ
> ダイオードが VDD_ESP + 0.3 V あたりに落とすので **3.6〜3.9 V 程度**に収まり、
> 自動リセット用トランジスタ（Q_EN / Q_IO0）の B-C 接合を通る漏れも **55〜100 µA** 程度で、
> ESP32 開発ボードで広く使われている構成でもある。**Rev.A はこれを受容する。**
> 1 枚だけ試すなら**ボッジ**として `U_USB`(CH340C) の **pin16(VCC) を持ち上げて VDD_ESP に繋ぎ、
> pin4(V3) を VCC に短絡**すれば CH340C ごと 3.3 V で動く。
> **Rev.B では CH340C を 3.3 V 駆動にする**（VCC=3.3 V・V3 を VCC に接続）。
...
| **VCAP1 の DC 電位（定常、AVSS 基準）を記録** | 期待値 **約 1.2 V**。**1.1 V を割っていたら RESET の条件（§11.1）を満たさないので要調査** |

**VCAP1 の誘電体（X5R）の実測確認**（lead 裁定 2026-09-23: C15008 / CL31A107MQHNNNE の X5R 100 µF のまま。
SBAS499C §11 は「多くの場合 VCAP1 も MLCC」としていて、X5R を避けるのは振動のある系に限った推奨。
ADS1299EVM と OpenBCI Cyton も X5R 100 µF を使っている。1.2 V での実効容量は約 90 µF、最悪でも 72 µF で、帯域は約 0.2 Hz。DS の要求は 10 Hz 未満）。

| 確認 | 方法 | 判断 |
|---|---|---|
| 入力短絡ノイズ・静置 | 全チャネル **MUX=001（入力短絡）**、机上に静置して数十秒記録 | 基準値として記録 |
| 入力短絡ノイズ・装着して動く | 同じ設定で、装着して頭を動かす | 静置と比べる |
| 入力短絡ノイズ・基板を叩く | 同じ設定で、基板を指やペン先で軽く叩く | 静置と比べる |
| 判定 | 叩いたときや動いたときにだけ出るステップ状・リンギング状のノイズ（圧電ノイズ）が見えるか | 出たら **Rev.B で C_VCAP1 をタンタル（A ケース）にすることを検討**し、記録を残す |
...
### 8-5. 電源ピンのバイパス経路（**afe-adc J1 の裁定の裏づけ。2026-09-24 追加**）
**背景**: J1 は緑だが、DVDD 48/50 の最寄りコンデンサ（C_DVDD_P40）までは F.Cu で 10.6〜13.7 mm あり、経路の L は 3〜14 nH と見積もった。AVDD 19/56/59 は In2 の AVDD ベタ経由（via 2 本）。机上では「ADC 自身の電流は fCLK 同期なので DC に畳まれる」「fMOD = 1.024 MHz では 100 nF のリアクタンス 1.55 Ω に対して経路は 0.02〜0.09 Ω」から、実害なしと判断した。ここではそれを実測で確かめる。

| # | 測定 | 合格値・判定 |
|---|---|---|
| 8-5a | **8-1 の内部短絡ノイズを 10 秒 × 3 回**（全 ch、250 SPS、gain 24、`CHnSET = 0x61`） | **3 回とも 8-1 の合格値（≤ 0.30 µVrms）**。比較の基準は SBAS499C の入力換算ノイズ表（Gain 24 / 250 SPS は typ 0.14 µVrms・0.98 µVpp、§7.5 は MAX 1.35 µVpp / 1 s） |
| 8-5b | **k·fMOD の折り返しトーン**: 8-5a のデータと、同じ設定で DR を最高（16 kSPS）にしたデータを FFT する | **DC と 50 Hz 系以外のスプリアスが、帯域内でノイズ床 +10 dB を超えない**（目安、推測）。スプリアスがあれば、その周波数 f_s から f_s = \|f_src − k·1.024 MHz\| を満たす f_src を探す（LM2664 の fSW 40–80 kHz 系、ESP32 の WiFi/クロック、USB）。根拠は SBAS499C p.24 *"fMOD = fCLK / 2"*、§9.3.2.1.1 *"pass band repeats itself at every fMOD"*、§11 p.70 *"noise that is nonsynchronous with the ADS1299-x operation must be eliminated"* |
| 8-5c | **DVDD のスパイク**: U_ADS pin 50 を、スプリング GND で pin 49（GND）に当てたプローブで見る。帯域 ≥ 200 MHz、SPI 読み出し中に SCLK の立ち上がりでトリガ | **記録する。机上の推定は 50〜100 mVpk（推測）**。**300 mVpk を超えるか、DVDD が 3.0 V を割るなら**、Rev.B の「全電源ピンについて via 0・2 mm 以内に 100 nF を 1 個」を前倒しで検討する（lead に上げる） |
...
| # | 確認 | 合格値 |
|---|---|---|
| 9-1 | ステータス LED（GPIO17）が点灯する | 目視で点灯（**R_LED = 330 Ω・黄 LED C84268 で約 3.9 mA**）。暗すぎて視認できないなら Rev.B で 220 Ω 前後を検討する（3.3 V GPIO のソース電流上限に注意） |
