...
発注は Via Covering = Tented の 4 層で、via を塞ぐ工程がない（2026-09-23 の Y9 投入時に JLC が Tented を選べなくし、無料で Plugged に置き換えた。Plugged はソルダーマスクのインクを穴に詰めるだけで、充填・キャップではない。JLC 自身も「パッド内の via は除く」としているので、I は covering にかかわらず 0 本を求める）。そのため 0402 の 29 か所、
